  ...
  return (
    <div className="fixed top-0 left-0 w-full z-[48] bg-n-8/80 backdrop-blur-sm border-b border-n-6">
      <div className="flex justify-between items-center px-5 lg:px-7.5 xl:px-12 max-lg:py-3">
        {/* <a
          className={`hidden max-lg:block absolute h-8 w-8 top-4 right-4 bg-n-2 z-50 rounded-md ${
            pathname === "/dashboard" ? "z-2 text-n-1" : "text-n-1/50"
          }`}
          href={"dashboard"}
        >
          <Icon size={1} path={mdiViewGridPlus} className="absolute left-1 top-1 text-n-9 z-2" />
        </a> */}
